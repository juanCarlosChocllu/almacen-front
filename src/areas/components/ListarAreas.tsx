...
export const ListarAreas = () => {
  ...
  return (
    <div className="overflow-x-auto shadow-lg rounded-lg border border-gray-300">
      { permisosModulo(permisos,ModulosE.AREAS, PermisoE.CREAR) &&    <CrearAreas recargar={recargarData} setRecargar={setRecargarData}/> }
        ...
        <tbody className="text-gray-800">
          {areas.map((item) => (
            <tr key={item._id} className="hover:bg-gray-100">
              ...
              { permisosModulo(permisos,ModulosE.AREAS, PermisoE.ELIMINAR) &&     <button onClick={()=>alertaDeEliminacion(()=> eliminar(item._id))} className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 focus:outline-none flex items-center">
                  <FaTrash className="mr-2" />
                </button> }
                { permisosModulo(permisos,ModulosE.AREAS, PermisoE.EDITAR) &&   <button onClick={()=> editar(item._id, item.nombre)} className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none flex items-center">
