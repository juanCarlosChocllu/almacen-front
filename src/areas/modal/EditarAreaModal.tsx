...
import { editarArea } from "../service/areasService";
...
export const EditarAreas = ({recargar,setRecargar,closeModal,idArea,isOpen,nombre}:{recargar:boolean,setRecargar:(recargar:boolean)=>void , nombre:string, idArea:string, isOpen:boolean, closeModal:()=>void}) => {
  ...
  const onSudmit = async (data: formAreasI) => {
    try {
      if (token) {
        const response = await editarArea(data, token,idArea);
